...
def bayesian_OSSP_qr(B_now, expected_prob_catch, type, num_alerts_count, attacker_type, lambda_p):
    ...
    try:
        ...
        for l in range(Attacker_type):
            ...
            for target in range(Target_num):
                ...
                obj.add(theta * z_l_list[target] * (R_l_d[1][target] * x[target][0] + R_l_d[0][target] * x[target][1]))
    ...
    # practice_obj_right = 0
    # for ele in range(Target_num):
    #     practice_obj_right += (solution[name[2] + str(ele)] + solution[name[3] + str(ele)]) * const_P[ele] * np.mean(num_alerts_count, axis = 1)[ele] * const_C[ele]
    # practice_obj = practice_obj_left + practice_obj_right
    ...
    return solution, z_list, model.objVal, practice_obj, opt_target, type_match_opt_type, run_time, usefulness